...
final class DefaultFileDownloader implements FileDownloader {
	...
	private CloseableHttpResponse executeViaProxy(Proxy proxy, String requestUrl) throws IOException {
		...
		LOGGER.info("HttpClient: " + proxyClient);

		final HttpHost proxyHttpHost = new HttpHost(proxy.host, proxy.port, proxy.protocol);

		LOGGER.info("HttpHost: " + proxyHttpHost);

		final RequestConfig requestConfig = RequestConfig.custom().setProxy(proxyHttpHost).setAuthenticationEnabled(true).build();

		final HttpGet request = new HttpGet(requestUrl);
		request.setConfig(requestConfig);
		LOGGER.info("HttpGet: " + request);
...
final class AuthenticatingFileDownloader implements FileDownloader {
	private static final Logger LOGGER = LoggerFactory.getLogger(FileDownloader.class);

	private final String username;
	private final String password;

	public AuthenticatingFileDownloader(String username, String password) {
	...
	private CloseableHttpResponse execute(String requestUrl) throws IOException {
		CloseableHttpResponse response;
		LOGGER.info("Downloading " + requestUrl + " for user: " + username);
